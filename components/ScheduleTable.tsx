    ...
    return (
        <div className="overflow-x-auto rounded-lg shadow-xl border border-gray-200 dark:border-zinc-800">
            <table className="min-w-full bg-white dark:bg-zinc-900 text-sm">
                ...
                <tbody>
                    {busStops.map((stop) => {
                        // Apply orange highlight if this is the next stop
                        const isNextStop = stop.is_next_stop && stop.estimated_arrival !== "N/A";
                        
                        const rowClass = isNextStop
                            ? "bg-orange-400 text-black font-semibold animate-pulse" // Orange highlight
                            : "text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-800";
